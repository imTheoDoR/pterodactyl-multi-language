...
class LocaleController extends Controller
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $requestedNamespaces = $request->input('namespace');
        $namespaces = [];

        if ($requestedNamespaces) {
            $namespaces = explode(' ', $requestedNamespaces);
        } else {
            $langPath = resource_path("lang/" . $locales[0]);
            ...
            foreach ($iterator as $file)
            {
                ...
                {
                    $relativePath = str_replace([$langPath . '/', '.php'], '', $file->getPathname());
                    $namespaces[] = str_replace('/', '.', str_replace('\\', '/', $relativePath));
                }
            }
        }
